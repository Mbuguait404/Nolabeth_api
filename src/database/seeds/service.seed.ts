...
import { TherapyServiceSchema } from '../../modules/services/schemas/therapy-service.schema';
...
export async function seedServices(connection: Connection) {
  const ServiceModel = connection.model('TherapyService', TherapyServiceSchema);
  
  await ServiceModel.deleteMany({});
  console.log('🗑️  Cleaned services collection');

  const created = await ServiceModel.insertMany(services);
