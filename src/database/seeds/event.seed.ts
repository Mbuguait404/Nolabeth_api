...
import { EventSchema, EventStatus, EventCategory } from '../../modules/events/schemas/event.schema';
...
export const upcomingEvents = [
  ...
  },
  {
    ...
    category: EventCategory.VIRTUAL,
    status: EventStatus.UPCOMING,
    features: [
...
export async function seedEvents(connection: Connection) {
  const EventModel = connection.model('Event', EventSchema);
  
  await EventModel.deleteMany({});
  console.log('🗑️  Cleaned events collection');

  const created = await EventModel.insertMany(upcomingEvents);
