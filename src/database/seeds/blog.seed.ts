...
import { BlogPostSchema } from '../../modules/blogs/schemas/blog-post.schema';
...
export async function seedBlogs(connection: Connection) {
  const BlogModel = connection.model('BlogPost', BlogPostSchema);
  
  await BlogModel.deleteMany({});
  console.log('🗑️  Cleaned blogs collection');

  const created = await BlogModel.insertMany(blogPosts);
